...
def estado_enemigo():
    for jugador in TODOS_LOS_JUGADORES:
        print(f"Jugador: {jugador.nombre} {"Muerto" if not jugador.estado else ""}{"Enemigo" if isinstance(jugador, Enemigo) else ""}")

def estado_medico():
    eliminados = 0
    for jugador in TODOS_LOS_JUGADORES:
        if jugador.estado == False:
            eliminados += 1
    print(f"\nMuertos: {eliminados}\n")
    for jugador in TODOS_LOS_JUGADORES:
        print(f"Jugador: {jugador.nombre} {"Medico" if isinstance(jugador, Medico) else ""}")

def estado_policia():
    print("\nEstado de los jugadores:")
    for jugador in TODOS_LOS_JUGADORES:
        print(f"Jugador: {jugador.nombre}, Estado: {'Vivo' if jugador.estado else 'Muerto'} {"Policia" if isinstance(jugador, Policia) else ""}")
